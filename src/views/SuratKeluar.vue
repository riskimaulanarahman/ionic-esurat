  ...
  <ion-page>
    <ion-header :translucent="true">
      <ion-toolbar>
        ...
        <ion-title>Surat Keluar</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content :fullscreen="true">
          ...
          <DxDataGrid
            ...
            <DxColumn 
            caption="no surat" 
            data-field="suratkeluar.no_surat"
            :editor-options="{disabled:true}"
            />
            <DxColumn 
            caption="perihal" 
            data-field="suratkeluar.perihal_surat"
            :editor-options="{disabled:true}"
            />
            <DxColumn 
            caption="tgl surat" 
            data-field="suratkeluar.tgl_dibuat"
            :editor-options="{disabled:true}"
            />
            <DxColumn 
            caption="no agenda" 
            ...
            <DxColumn 
            caption="sifat surat" 
            data-field="suratkeluar.sifat_surat"
            :editor-options="{disabled:true}"
            />
            <DxColumn 
...
const store = new CustomStore({
	...
	load: function() {
      return axios.get(API_URL + 'alluser').then(responsea => {
        return axios.get(API_URL + 'checknik/'+responsea.headers.email).then(responseb => {
          return axios.get(API_URL + 'disposisi-ionic?module=suratkeluar&nik='+responseb.data.nik).then(function(response){
	...
	update: (key, values) => {
      ...
      return axios.put(API_URL +'disposisi-ionic'+"/"+key,{ module : "suratkeluar" ,data : values }).then(function(response){
			Toast.fire({
				icon: response.data.status,
				title: response.data.message
			})
